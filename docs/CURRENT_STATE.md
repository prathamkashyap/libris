...
- **Phase 5A demand forecasting** (`scripts/dev-seed/phase5a_demand_aggregation.py`): Monthly demand aggregation baseline with seasonal-naive forecasting. Corrected historical lookup to preserve monthly dimension (item_type|category|month). Synthetic temporal validation (train 2023-01 through 2025-12, test 2026-01 through 2026-08): MAE = 2.7933, RMSE = 4.2171 (208 forecast points). Demand target: ALL borrow events by borrowDate regardless of return status. No production integration.
- **Phase 5B demand forecasting baselines** (`scripts/dev-seed/phase5b_calendar_baselines.py`): Offline synthetic-data development prototype extending Phase 5A with three baselines: seasonal naive, trailing 3-month moving average, trailing 6-month moving average. Static holdout temporal validation (train 2023-01 through 2025-12, test 2026-01 through 2026-08). Synthetic academic-calendar regime analysis based on semester_factor() (BREAK/NORMAL/REDUCED months). Calendar regimes are SYNTHETIC development assumptions only and do not represent real academic calendar data. Test results on synthetic data (libris_ml_dev, 9,575 borrow events, 26 series): Seasonal naive MAE 2.7933/RMSE 4.2171; 3-month MA MAE 3.1886/RMSE 4.7722; 6-month MA MAE 3.2042/RMSE 4.8113 (208 forecast points per baseline). No production integration.
