...
public class NewspaperService {
    ...
    public Page<NewspaperResponse> list(String search, Pageable pageable) {
        var source = search == null || search.isBlank() ? newspapers.findAll(pageable)
    ...
    public void delete(Long id) {
        var entity = newspaper(id);
        if (records.existsByNewspaperId(id)) throw new ConflictException("A newspaper with borrow history cannot be deleted.");
        newspapers.delete(entity);
    }

    private Newspaper newspaper(Long id) {
        return newspapers.findById(id).orElseThrow(() -> new ResourceNotFoundException("Newspaper not found."));
    }
    ...
    private NewspaperResponse response(Newspaper n) {
        return new NewspaperResponse(n.getId(), n.getTitle(), n.getPublisher(), n.getPublicationDate(), n.getTopHeadlines(), n.isAvailable());
    }
}
