...
public class BorrowRecordService {
    ...
    public BorrowRecordService(BorrowRecordRepository r, BookRepository b, MagazineRepository m, NewspaperRepository n, StudentProfileRepository s, ApplicationEventPublisher events, CurrentUser currentUser) {
        records = r; books = b; magazines = m; newspapers = n; students = s; this.events = events; this.currentUser = currentUser;
    }
    ...
    public BorrowRecordResponse borrow(BorrowRequest r) {
        long selectedItems = java.util.stream.Stream.of(r.bookId(), r.magazineId(), r.newspaperId())
        ...
        events.publishEvent(new EntityAuditEvent(this, AuditAction.BORROW, AuditEntityType.BORROW_RECORD,
                saved.id(), "Borrowed: " + itemTitle + " by " + student.getName(),
                actor.id(), actor.username(), actor.role(), actor.ipAddress(), actor.userAgent()));
        return saved;
    }
    ...
    public void returnBook(Long id) {
        ...
        String itemTitle = null;
        if (record.getBook() != null) { record.getBook().setAvailable(true); itemTitle = record.getBook().getTitle(); }
        if (record.getMagazine() != null) { record.getMagazine().setAvailable(true); itemTitle = record.getMagazine().getTitle(); }
        if (record.getNewspaper() != null) { record.getNewspaper().setAvailable(true); itemTitle = record.getNewspaper().getTitle(); }

        var actor = currentUser.get();
        events.publishEvent(new EntityAuditEvent(this, AuditAction.RETURN, AuditEntityType.BORROW_RECORD,
                id, "Returned: " + (itemTitle != null ? itemTitle : "item"),
                actor.id(), actor.username(), actor.role(), actor.ipAddress(), actor.userAgent()));
    }
