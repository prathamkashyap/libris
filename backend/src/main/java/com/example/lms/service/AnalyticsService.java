...
public class AnalyticsService {
    ...
    public AnalyticsService(BookRepository books, StudentProfileRepository students,
                            LibrarianProfileRepository librarians, BorrowRecordRepository borrowRecords) {
        this.books = books;
    ...
    public AnalyticsDashboardResponse dashboard() {
        ...
        var overdueCount = borrowRecords.findByReturnDateIsNullAndBorrowDateBefore(LocalDate.now().minusDays(14)).size();
        return new AnalyticsDashboardResponse(totalBooks, totalStudents, totalLibrarians,
                borrowedBooks, availableBooks, overdueCount);
    }
    ...
    public OverdueSummaryResponse overdue() {
        ...
        var items = borrowRecords.findByReturnDateIsNullAndBorrowDateBefore(cutoff).stream()
                .map(r -> new OverdueSummaryResponse.OverdueItem(
                        r.getId(),
                        itemTitle(r),
                        r.getBorrowerName(),
                        r.getBorrowDate(),
