...
public class ReportService {
  ...
  public String inventoryCsv() {
    ...
    return Stream.concat(
            Stream.of(header), Stream.concat(bookRows, Stream.concat(magazineRows, newspaperRows)))
            ...
            .map(
                r -> {
                  var itemType =
                      r.getBook() != null
                          ? "BOOK"
                          : r.getMagazine() != null ? "MAGAZINE" : "NEWSPAPER";
                  var itemTitle =
                      r.getBook() != null
                          ? r.getBook().getTitle()
                          : r.getMagazine() != null
        ...
        allBorrows.stream()
            .collect(
                Collectors.groupingBy(
                    r -> r.getStudent() != null ? r.getStudent().getId() : -1L,
