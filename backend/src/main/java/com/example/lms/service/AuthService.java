...
public class AuthService {
    ...
    public AuthService(AuthenticationManager authenticationManager, AccountRepository accounts, ApplicationEventPublisher events, CurrentUser currentUser) {
    ...
    public AuthenticatedUserResponse login(LoginRequest request, HttpServletRequest servletRequest) {
        ...
        events.publishEvent(new EntityAuditEvent(this, AuditAction.LOGIN, AuditEntityType.ACCOUNT,
                account.getId(), "User logged in: " + account.getUsername(),
                account.getId(), account.getUsername(), account.getRole().name(),
                actor.ipAddress(), actor.userAgent()));

        return new AuthenticatedUserResponse(account.getId(), account.getUsername(), account.getRole().name(), account.getUsername());
    }
    ...
    public void logout(HttpServletRequest request) {
        ...
        if (account != null) {
            events.publishEvent(new EntityAuditEvent(this, AuditAction.LOGOUT, AuditEntityType.ACCOUNT,
                    account.getId(), "User logged out: " + account.getUsername(),
                    account.getId(), account.getUsername(), account.getRole().name(),
                    actor.ipAddress(), actor.userAgent()));
        }
