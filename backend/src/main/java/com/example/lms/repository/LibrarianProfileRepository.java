...
public interface LibrarianProfileRepository extends JpaRepository<LibrarianProfile, Long> {
  @Query(
      "SELECT l FROM LibrarianProfile l WHERE LOWER(l.name) LIKE LOWER(CONCAT('%',:q,'%')) OR LOWER(l.account.username) LIKE LOWER(CONCAT('%',:q,'%'))")
  org.springframework.data.domain.Page<LibrarianProfile> search(
