...
@RestController @RequestMapping("/api/reports") @Tag(name="Reports",description="CSV export endpoints for inventory, borrowing, and student data")
public class ReportController {
    private final ReportService service;
    public ReportController(ReportService s) { this.service = s; }

    @GetMapping("/inventory") @Operation(summary="Inventory report",description="Full catalogue inventory (books + magazines + newspapers) as CSV.")
    ...
    @GetMapping("/borrowing") @Operation(summary="Borrowing report",description="All borrow records with optional date range filter, as CSV.")
    ...
    @GetMapping("/overdue") @Operation(summary="Overdue report",description="All overdue items with days overdue, as CSV.")
    ...
    @GetMapping("/students") @Operation(summary="Student report",description="All students with total and active borrow counts, as CSV.")
